...
def get_authorization_url(redirect_uri: str) -> Optional[Tuple[str, str, Optional[str]]]:
    """認証URL・state・code_verifier を取得（OAuthコールバック用）"""
    if not GOOGLE_API_AVAILABLE:
        return None
...
def exchange_code_for_credentials(
    ...
    if flow is None:
        return None
    flow.fetch_token(code=code, state=state, code_verifier=code_verifier)
